...
- `Shell` accepts a quadrilateral mesh, which uses the MITC4 transverse shear interpolation of Dvorkin and Bathe and therefore does not lock in the thin limit. The element type follows from the connectivity, as it does for `Planar`, and `import_mesh(...)` reads a non-planar quadrilateral mesh as a `Shell`.
...
- **Breaking:** `drill_penalty` of `Shell` ties the drilling degree of freedom to the in-plane rotation of the membrane field rather than penalizing it towards zero, and is a fraction of the shear stiffness of the section rather than an absolute stiffness, defaulting to `1e-3`. Penalizing the drilling rotation itself resists the rigid rotation that differing element normals spread over the drilling degrees of freedom, which locked a doubly-curved shell: the pinched hemisphere now stays within 1% on a 16x16 mesh where it reached 13% of the reference before.
