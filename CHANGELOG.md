...
- **Breaking:** `drill_penalty` of `Shell` is a fraction of the shear stiffness of the section rather than an absolute stiffness, and defaults to `1e-3`. An absolute penalty had to be matched to the material and the mesh by hand, and the previous default of `1.0` was effectively zero for a stiff shell, which let the drilling degrees of freedom drift and kept a curved shell from converging under refinement. The Scordelis-Lo roof now converges to its reference instead of overshooting it by 6% on a 32x32 mesh.
