...
- `solve(..., nlgeom=True)` raises `NotImplementedError` up front on a model that does not carry geometric nonlinearity, which `Truss` accepted silently while solving on the reference configuration, where the direction cosines of the undeformed bar keep a rotation out of the axial strain. `Shell` rejected it already, but only from inside the Newton loop, whose cutbacks caught it as the `RuntimeError` it derives from and reported a spurious non-convergence after ten retries.
