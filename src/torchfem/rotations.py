...
def voigt_stress_rotation(R: Tensor) -> Tensor:
    ...
    if R.shape[-1] == 2 and R.shape[-2] == 2:
        ...
        return torch.stack(
            [
                torch.stack([R00**2, R01**2, 2 * R01 * R00], dim=-1),
                torch.stack([R10**2, R11**2, 2 * R11 * R10], dim=-1),
                torch.stack([R00 * R10, R01 * R11, R00**2 - R10**2], dim=-1),
            ],
            dim=-1,
        )
...
def voigt_strain_rotation(R: Tensor) -> Tensor:
    if R.shape[-1] == 2 and R.shape[-2] == 2:
        ...
        return torch.stack(
            [
                torch.stack([R00**2, R01**2, R01 * R00], dim=-1),
                torch.stack([R10**2, R11**2, R11 * R10], dim=-1),
                torch.stack([R00 * R10, R01 * R11, R00**2 - R10**2], dim=-1),
            ],
            dim=-1,
        )
