...
class Assembly:
    ...
    def coupling(
        ...
        r = skew(x_secondary - primary.nodes[primary_nodes], self.dim)
        ...
        for dof in dofs:
            ...
            if dof < self.dim and n_pri > self.dim:
                for b, axis in enumerate(axes):
                    ...
                    self._rows.append((secondary_base + dof, rot, r[:, dof, axis]))
    ...
    def _near_null_space(self) -> Tensor:
        ...
        The parts share one physics and one dimension, so their blocks carry the
        same number of modes and stack.
        """
        return torch.cat(
